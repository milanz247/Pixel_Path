...
export function HowItWorksSection() {
  return (
    <SectionWrapper background="offwhite" id="how-it-works" divider={false}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-8 relative z-10">
          {steps.map((step, index) => {
            ...
            return (
              <div
                key={index}
                className="animate-on-scroll bg-white rounded-2xl p-6 lg:p-8 flex flex-col relative border border-[#E5E5E0] shadow-sm transform transition-all duration-300 hover:shadow-lg hover:-translate-y-1"
              >
                {/* Number Pill */}
                <div className="absolute top-5 left-5 bg-[#F8F8F6] border border-[#E5E5E0] text-[#1C1C1E] font-bold text-xs rounded-full px-3 py-1 font-heading">
                  {step.num}
                </div>

                <div className="flex flex-col items-center text-center mt-6">
                  {/* Icon */}
                  <div className="w-14 h-14 rounded-2xl bg-[#E8F5EE] flex items-center justify-center mb-6">
      ...
      <div className="animate-on-scroll text-center bg-white py-6 px-4 rounded-xl border border-[#E5E5E0] shadow-sm max-w-3xl mx-auto">
